...
function cp_editor( $content, $args = array() ) {
	...
	$textarea_field = html( 'textarea', $args, esc_textarea( $content ) );

	ob_start();
	wp_editor( $content, $args['id'], cp_get_editor_settings() );
	$editor = ob_get_clean();

	echo preg_replace( '#<textarea[^>]*>(.*?)</textarea>#is', $textarea_field, $editor );
}
